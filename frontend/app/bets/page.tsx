import { AlertTriangle, Brain, CalendarDays, ExternalLink, Layers, RotateCcw } from "lucide-react";
import { BankrollTimeline } from "@/components/BankrollTimeline";
import { FakeBetSlipCard } from "@/components/FakeBetSlipCard";
import { LiveBankrollTracker } from "@/components/LiveBankrollTracker";
...
export default async function BetsPage() {
  ...
  const pendingMultiSlips = multiSlips.filter((slip) => slip.status === "pending-live");
  const multiRisk = pendingMultiSlips.reduce((total, slip) => total + slip.stake, 0);
  const multiReturn = pendingMultiSlips.reduce((total, slip) => total + slip.potential_return, 0);
  ...
  return (
    <div className="space-y-8">
      ...
      <LiveBankrollTracker initialChallenge={challenge} />
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-2xl font-black">Multi Bet Audit</h2>
            </div>
            <p className="mt-1 text-sm text-slate-600">New multis are paused in recovery mode. Settled multis remain visible so the model cannot hide the damage.</p>
          </div>
          <span className="rounded bg-white px-3 py-2 text-xs font-bold uppercase tracking-wide text-coral">paused after losses</span>
        </div>
        <div className="mb-4 grid gap-3 sm:grid-cols-3">
          <Metric label="Pending multi risk" value={`$${multiRisk.toFixed(2)}`} />
          <Metric label="Pending multi return" value={`$${multiReturn.toFixed(2)}`} />
